...
public class PrefabLightmapData : MonoBehaviour
{
    ...
    static void ApplyRendererInfo(RendererInfo[] infos, int[] arrayOffsetIndex)
    {
        for (int i = 0; i < infos.Length; i++)
        {
            ...
            {
                info.terrain.lightmapIndex = arrayOffsetIndex[info.lightmapIndex];
    ...
    static void GenerateLightmapInfo_MeshRender()
    {
        GenerateLightmapInfo(0);
        ...
        //resourcePath = resourcePath.Replace("/Lightmaps/", "/" + sceneName + "_Lightmaps/");
        //Debug.Log("resourcePath " + resourcePath);
        //Debug.Log("scenePath " + scenePath);

        //string lightMapPath = System.IO.Path.Combine(Directory.GetCurrentDirectory(), resourcePath);
        //if (!Directory.Exists(lightMapPath))
        //    Directory.CreateDirectory(lightMapPath);

        //PrefabLightmapData[] prefabs = FindObjectsOfType<PrefabLightmapData>();
        //Debug.Log("prefabs count " + prefabs.Length);
        //foreach (var instance in prefabs)
        //{
        ...
        //    {
        //        //Prefab
        //        Debug.Log("targetPrefab != null " + targetPrefab.name);
    ...
    [MenuItem("Assets/Bake terrain Lightmaps")]
    static void GenerateLightmapInfo_Terrain()
    {
        GenerateLightmapInfo(1);
    }

    static void GenerateLightmapInfo(int genType)
    {
        Debug.ClearDeveloperConsole();
        ...
        foreach (var instance in prefabs)
        {
            ...
            GenerateLightmapInfo(scenePath, resourcePath, gameObject, rendererInfos, lightmaps, lightmaps2, genType);
    ...
    static void GenerateLightmapInfo(string scenePath, string resourcePath, GameObject root, List<RendererInfo> rendererInfos, List<Texture2D> lightmaps, List<Texture2D> lightmaps2, int genType = 0)
    {
        if (genType == 0)
        {
        ...
        foreach (Terrain terrain in terrains)
        {
            if (terrain.lightmapIndex != -1)
            {
                ...
                Texture2D lightmap = LightmapSettings.lightmaps[terrain.lightmapIndex].lightmapColor;
                Texture2D lightmap2 = LightmapSettings.lightmaps[terrain.lightmapIndex].lightmapDir;
    ...
    {
        Debug.Log(filename + " " + File.Exists(filename));
